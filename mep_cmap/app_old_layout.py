...
class TMSAnalysisApp(Stage2Mixin, FilterPreviewMixin, BidsifyTabMixin):
    ...
    def _build_scrollable_container(self):
        ...
        # ── Add-ons (added last; preserves the hardcoded tab indices 2/3) ─
        self.tab_addons = ttk.Frame(self.notebook)
        self.notebook.add(self.tab_addons, text="Add-ons")
        ...
        self.notebook.bind(
            ...
            add="+")
        # Rescan add-ons whenever the Add-ons tab is shown (fresh drop-ins).
        self.notebook.bind(
            "<<NotebookTabChanged>>", self._on_addons_tab_selected, add="+")
    ...
    def _addons_discover(self):
        ...
        try:
            self._addons_entries = _addons.discover_all(prefs.addons_path,
                ...
                for sp in entry["settings"]:
                    ...
                    if sp.get("help"):
                        tk.Label(sframe, text=sp["help"], fg="grey", justify="left",
                                 wraplength=600, font=("TkDefaultFont", 8)).pack(anchor="w")
    ...
    def _on_addons_tab_selected(self, event=None):
        """Rescan add-ons whenever the Add-ons tab is selected."""
        try:
            if self.notebook.select() == str(self.tab_addons):
                self._addons_refresh_status()
                self._addons_discover()
        except Exception:
            pass
