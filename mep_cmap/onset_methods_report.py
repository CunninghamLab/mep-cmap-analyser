...
When onset-method agreement is enabled, every consensus member runs on every
...
``consensus`` without enabling agreement produces no comparison outputs, and
the pipeline says so rather than writing nothing silently.
  ...
  * plot_bland_altman           -- each method vs the others' consensus
...
METHOD_COLS = [
    ...
    "Latency(ms)", "Detected", "Delta_From_Consensus(ms)",
    # Consensus recomputed WITHOUT this method. See _loo_consensus.
    "LOO_Consensus(ms)", "Delta_From_LOO_Consensus(ms)",
]
...
SUMMARY_COLS = [
    ...
    "Mean_Delta_From_Consensus(ms)", "SD_Delta_From_Consensus(ms)",
    "LoA_Lower(ms)", "LoA_Upper(ms)",
    # Bland-Altman statistics against the leave-one-out consensus, which is
...
def _loo_consensus(per_method, exclude):
    """Median of the other members, excluding ``exclude``.

    A Bland-Altman plot of a method against the consensus is a part-whole
    comparison: the method being assessed is one of the values the consensus
    is computed from, so it is correlated with its own reference. That pulls
    ...
    Recomputing the consensus without the method under test removes that.
    ...
    single remaining value is not a consensus.
    """
...
def collect_agreement_rows(agreement_by_trial, name, custom_labels=None):
    ...
    for (stim_type, idx), ag in sorted(
        ...
        if ag is None:
            continue
        consensus = ag.consensus_ms
        # The consensus is a DERIVED value, not a member: compute_onset_agreement
        ...
        per_method[CONSENSUS_KEY] = consensus
        for method, latency in sorted(per_method.items()):
            delta = None
            if latency is not None and consensus is not None:
                delta = round(float(latency) - float(consensus), 3)
            # For the consensus row itself the leave-one-out reference is the
            # median of all members, i.e. the consensus -- so the difference is
            # zero by construction and carries no information.
            loo = (None if method == CONSENSUS_KEY
                   else _loo_consensus(ag.per_method, method))
            loo_delta = None
            ...
            rows.append({
                ...
                "Delta_From_Consensus(ms)": delta,
                "LOO_Consensus(ms)": round(loo, 3) if loo is not None else None,
                "Delta_From_LOO_Consensus(ms)": loo_delta,
            })
    return rows
...
def build_method_summary(rows):
    ...
    difference from the consensus (mean +/- 1.96 SD). They answer the question
    ...
    for (fname, stim, label, method), g in df.groupby(
        ...
        dlt = pd.to_numeric(g["Delta_From_Consensus(ms)"],
                            errors="coerce").dropna()
        loo = pd.to_numeric(g["Delta_From_LOO_Consensus(ms)"],
                            errors="coerce").dropna()
        n_tot = len(g)
        ...
        out.append({
            ...
            "Mean_Delta_From_Consensus(ms)": round(d_mean, 3) if d_mean is not None else None,
            "SD_Delta_From_Consensus(ms)": round(d_sd, 3) if d_sd is not None else None,
            "LoA_Lower(ms)": loa_lo,
            "LoA_Upper(ms)": loa_hi,
...
def plot_bland_altman(rows, figures_out, bids_prefix, use_loo=True):
    """Bland-Altman of each method against the consensus of the others.
    ...
    The reference is the LEAVE-ONE-OUT consensus by default: the method under
    ...
    if not rows:
        return None
    df = pd.DataFrame(rows)
    diff_col = ("Delta_From_LOO_Consensus(ms)" if use_loo
                else "Delta_From_Consensus(ms)")
    ref_col = "LOO_Consensus(ms)" if use_loo else None
    ...
    methods = [m for m in sorted(df["Method"].unique()) if m != "consensus"]
    if not methods:
        return None
    stims = list(dict.fromkeys(df["StimType"]))
    ...
    ref_name = ("leave-one-out consensus" if use_loo else "consensus")
